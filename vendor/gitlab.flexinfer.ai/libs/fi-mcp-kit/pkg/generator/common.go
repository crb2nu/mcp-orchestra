...
// as-is for runtime resolution by the client tools or wrappers.
//
// repoPath: The workspace root (e.g. ~/workspace).
...
func ResolveCommand(cmd string, repoPath string, context string) string {
	...
	if context == "local" {
		if strings.HasPrefix(resolved, "scripts/") ||
			strings.HasPrefix(resolved, "mcp/") ||
